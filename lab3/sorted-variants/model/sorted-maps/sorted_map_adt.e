...
feature -- commands
	...
	prune_tolerant (val: V; i: INTEGER)
		...
		do
			...
			pruned: (old occurrences (val).count >= i) implies model ~ (old model.deep_twin @<< (((old occurrences (val)).deep_twin) [i]))
				-- To Do
	...
	merge (other: SORTED_MAP_ADT [K, V])
		...
		require
			...
			-- Hint: Consider the intersection operator |/\| in the SET class in Mathmodels.
				SET.intersected (other).count ~ Current.count + other.count
		deferred
		ensure
			merged: False
			-- To Do
