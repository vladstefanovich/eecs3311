...
feature -- position queries
	...
	sibling: detachable like Current
			...
			-- Returns nothing if child is root or has no sibling
		do
			-- TO DO --

			if attached parent as p then
				if attached p.left as l and then attached p.right as r then
					if is_equal(l) then
						Result := r
					else
						Result := l
	...
	inner_child: detachable like Current
		...
		do
			...
			if attached parent as p then
				if attached p.left as l and attached p.right as r then
					if is_equal(r) then
						Result := r.left
					else
						Result := l.right
...
feature -- inorder traversal from Current
	...
	traverse_inorder
		...
		do
			-- TO DO --
				...
				if attached Current.left as l and then attached Current.right as r then
					l.traverse_inorder
					r.traverse_inorder
					inorder_result := l.inorder_result + Current.out + r.inorder_result
				end
	...
	inorder: STRING
		...
		do
			Result := out
			-- TO DO --
			if attached Current as curr then
				if attached curr.left as l then
					Result := l.inorder + Current.out
				end
				if attached curr.right as r then
					Result := r.inorder + Current.out
				end
			end
