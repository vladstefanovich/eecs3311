...
--				Result := l_root
--			end
			create l_node.make (a_node.key,a_node.value)

			if attached a_node.left as al then
				if find_largest(al) > l_node then
					l_node := al
				end
			end
			if attached a_node.right as ar then
				if find_largest(ar) > l_node then
					l_node := ar
				end
			end
